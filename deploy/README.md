...
填写 Server 的纯 `host:port` 地址（不要写 `https://`）、注册令牌、宿主机源目录和宿主机恢复目录。UI 与 REST 中始终填写宿主机路径；Agent 仅在执行边界将其转换为容器运行时路径。Docker 常规部署只需设置 `BMC_RESTORE_ROOT`。启动：

```sh
docker compose --env-file .env.agent -f deploy/docker-compose.agent.yml up -d --build
```

每台 Agent 必须在实际拥有备份源目录和恢复目录的主机上运行。默认显式只读挂载宿主机 `/etc`、`/srv`，映射为容器 `/backup-sources/etc`、`/backup-sources/srv`；`BMC_SOURCE_PATH_MAPPINGS` 的每一项都必须对应一个只读挂载，并且映射后的路径必须包含在 `BMC_SOURCE_ROOTS` 中。Docker 常规部署会将 `BMC_RESTORE_ROOT` 的宿主机路径挂载并自动映射到容器 `/backup-restore`，该容器路径自动成为唯一允许恢复目标。

需要多个恢复目录、非标准容器挂载布局或 bare-metal 时，显式设置 `BMC_RESTORE_PATH_MAPPINGS` 与 `BMC_RESTORE_ROOTS`。两者存在时按显式值覆盖 `BMC_RESTORE_ROOT` 的自动映射和白名单；每项映射都必须对应一个读写挂载，映射后的路径必须包含在 `BMC_RESTORE_ROOTS` 中。禁止挂载整个主机根目录或 `/var/run/docker.sock`。
