...
func TestRunForget_SnapshotIDs_Succeeds(t *testing.T) {
	...
	if err != nil { t.Fatalf("runForget: %v", err) }
	if res == nil { t.Fatal("expected result") }
	want := []string{"forget", "abc123def456", "--prune", "--repo", "rclone:remote:/repo"}
	if len(cmd.Args) < len(want) { t.Fatalf("args too short: %q", cmd.Args) }
	for i, w := range want {
		if cmd.Args[i] != w { t.Fatalf("args[%d] = %q, want %q", i, cmd.Args[i], w) }
	}
}
...
func TestRunForget_SnapshotIDs_RejectsTags(t *testing.T) {
	...
	if !errors.As(err, &pErr) { t.Fatalf("expected PipelineError, got %T", err) }
	if pErr.Code != "invalid_params" { t.Fatalf("code = %q, want invalid_params", pErr.Code) }
}
...
func TestRunForget_SnapshotIDs_ResticFailureMapsToForgetFailed(t *testing.T) {
	...
	var pErr *PipelineError
	if !errors.As(err, &pErr) { t.Fatalf("expected PipelineError, got %T", err) }
	if pErr.Code != "forget_failed" { t.Fatalf("code = %q, want forget_failed", pErr.Code) }
}
