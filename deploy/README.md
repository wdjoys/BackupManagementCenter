...
Compose 部署请阅读 [`deploy/docker-compose.md`](docker-compose.md)。

- Server：使用仓库根目录的 `docker-compose.server.yml`。
- Agent：在每台受管 Linux 主机上单独使用 `docker-compose.agent.yml`。
- Docker 镜像构建文件：`Dockerfile.server`、`Dockerfile.agent`。
***
